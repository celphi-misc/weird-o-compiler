...
typedef enum{
    ...
    AND_ASS, OR_ASS, DOT_ASS, BRACKET_ASS,
    // binary operations (I)
    ...
    ANDI_ASS, ORI_ASS, BRACKETI_ASS,
    // branch and jump (J)
    BEQ_ASS, BNE_ASS, BNZ_ASS, 
    JMP_ASS, JA_ASS, JR_ASS,
} Instruction;
...
struct assemble_instruction_t{
    ...
    instType type;
    Instruction inst;
    union { Reg reg; Lab lbl; } fac1;
    union { Reg reg; Val val; Lab lbl; } fac2;
    union { Reg reg; Val val; Lab lbl; } fac3;
};
...
int processNodeCjump(TreeNode node);
